  ...
  return (
    <div className="min-h-screen bg-[#fafaf9] text-[#1c1917]">
      ...
      <div className="max-w-2xl mx-auto px-6 pb-10">
        <div className="mb-3 text-[11px] font-semibold tracking-[2px] uppercase text-[#57534e]">
          Contents
        </div>

        <div className="space-y-px rounded-3xl border border-[#e7e5e4] bg-white overflow-hidden">
          {[
            { num: "01", title: "The State Does Not Need to Affirm Your Version of Love", id: "arg-1" },
            { num: "02", title: "Marriage Has a Real Definition", id: "arg-2" },
            { num: "03", title: "Children Need Mothers and Fathers", id: "arg-3" },
            { num: "04", title: "Redefining Marriage Changes It for Everyone", id: "arg-4" },
            { num: "05", title: "If It’s Just Feelings, Marriage Becomes Meaningless", id: "arg-5" },
          ].map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className="group flex items-center gap-4 px-5 py-4 text-[15px] border-t border-[#e7e5e4] first:border-t-0 hover:bg-[#f8f7f6] transition-all active:bg-[#f1f0ed]"
            >
              <div className="font-mono text-[21px] font-semibold text-[#3b2a6e] w-8 tabular-nums tracking-tighter group-hover:text-[#2a1f52] transition-colors">
                {item.num}
              </div>
              <div className="flex-1 text-[#1c1917] group-hover:text-[#3b2a6e] leading-snug transition-colors">
                {item.title}
              </div>
              <div className="text-[#c4b8a8] group-hover:text-[#3b2a6e] transition-colors">→</div>
            </a>
          ))}
        </div>
      </div>
